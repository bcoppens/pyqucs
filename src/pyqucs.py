...
def tolerance_range(nominal_value):
    component_value = nominal_value.value
    delta = component_value * float(nominal_value.tolerance) / 100.0
    return (component_value - delta, component_value + delta)

def equally_spaced(nominal_value, nr_samples):
    range = tolerance_range(nominal_value)
    step = float(range[1] - range[0]) / nr_samples
    return np.arange(range[0], range[1], step=step)

def uniform(nominal_value, nr_samples):
    range = tolerance_range(nominal_value)
...
def normal(nominal_value, sigma, nr_samples):
    return np.random.normal(nominal_value.value, sigma, nr_samples)

def create_normal(sigma):
    return lambda nominal_value, nr_samples: normal(nominal_value, sigma, nr_samples)
...
def set_component_to_random_sample(netlist, component_name):
    ...
    component.value.value = component.sample(1)[0]
